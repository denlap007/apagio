...
const goTools = path.join(__dirname, "../../tools/go");
// Add the project's required go tools to the PATH.
const devPath = `${process.env.PATH}:${goTools}/bin`;
...
function spawnProcess(processName, args, envOverride) {
  const deferred = q.defer();
  const envLocal = lodash.merge(env, envOverride);
  const goTask = child.spawn(processName, args, {
    env: envLocal,
    stdio: "inherit",
  });
